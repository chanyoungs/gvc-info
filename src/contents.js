...
const fetchData = async () => {
    ...
    const rows = await csvtojson().fromString(response.data);
    const formattedData = {};

    rows.forEach((row, index) => {
        Object.keys(row).forEach((key) => {
            const [keyName, lang] = key.split("-");
            if (!formattedData[keyName]) {
                formattedData[keyName] = { kor: {}, eng: {} };
            }
            if (index === 0) {
                formattedData[keyName][lang].title = row[key];
                formattedData[keyName][lang].items = [];
            } else if (formattedData[keyName][lang] && row[key]) {
                formattedData[keyName][lang].items.push(row[key]);
            }
        });
    });
...
const contentsNoIndex = [
        ...
        content: (
            <>
                <Typography variant="body1">
                    ...
                    10회권(무기한), 정액권도(최소 2주 Freeze/Unfreeze Membership
                    가능) 있으니까 참고 하시고 일일권은 시간당이 아니라 하루
                    이용권입니다. 한 센터에 입장권이 있으시면 다른 Arch
                    센터들에서도 입장 가능합니다.
                </Typography>
                <Typography variant="body1" align="center">
                    <CustomLink
    ...
    },
    {
        ...
        content: (
            <>
                <Typography variant="body1">
                    Arch Climbing 처음 오시는분들은 안전교육 영상(3분)을 미리
                    숙지하시고 온라인 등록 하신후 카운터에서 등록 확인 및
                    안전교육 관련 확인 질문들 답해주시면 됩니다. 등록할때 "Where
                    have you bouldered before inside or outside?" 질문에
                    클라이밍 처음이실 경우는 그냥 "First time" 이라고 적으시면
                    돼요.
                </Typography>
                <br />
                <Typography variant="body1">
                    클라이밍 자체를 처음 해보시는 분은 클럽장에게 미리
                    알려주시면 저희가 직접 인덕션 및 기본 코칭 해드릴수 있으니
                    미리 연락 주세요.
                </Typography>
                <Typography variant="body1" align="center">
                    <CustomLink
            ...
            <>
                <Typography variant="body1" align="center">
                    <CustomLink
                        ...
                        color="secondary"
                    >
                        ClimbTourTV_클라임투어티비 - 클라이밍 기초 Youtube
                        Playlist Link
                    </CustomLink>
                </Typography>
                <Typography variant="body1">
